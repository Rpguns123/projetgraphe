...
public class Fleury {
	...
	/*
	 * Fonction qui test dans le graphe si il existe un chemin de i � j
	 * Graphe f : graphe dans lequel chercher le chemin
	 * int i,j : point de d�part et d'arriv� du chemin recherch�
	...
	static boolean existeChemin(Graphe f, int i, int j, ArrayList<Integer> ch, int d) {
		int tmp = -1; 
		ch.add(d);
		// Si ch ne contient qu'un �l�ment, il faut eviter le chemin ij qui est l'arete ij. Similaire au fait de supprimer l'arrete ij du graphe f
		if(ch.size()==1){
		...
		else{//Cas ou ch contient plus d'un �l�ment
